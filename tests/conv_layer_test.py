...
from models.conv_layer import ConvBnRelu


def test_basic_conv_layer():
    in_channels, out_channels = 3, 9
    kernel_sz, stride, pad = 3, 1, 1
    batch_sz, ysz, xsz = 4, 9, 9
    layer = ConvBnRelu(in_chan=in_channels, out_chan=out_channels, kernel=kernel_sz, stride=stride, pad=pad)
...
def test_grad_conv_layer():
    layer = ConvBnRelu(in_chan=1, out_chan=3)
